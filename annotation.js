...
class AnnotationManager {
    ...
    positionContent(content, marker, direction) {
        ...
        const minWidth = isSmallScreen ? 250 : 280;
        const maxWidth = isSmallScreen ? 
            Math.min(320, viewportWidth * 0.9) : // 小屏幕时最多占90%宽度
            Math.min(450, viewportWidth - 40);   // 大屏幕时限制最大宽度
        
        // 高度也根据视口高度调整
        const maxHeight = Math.min(400, viewportHeight * 0.7); // 最多占视口高度的70%
        ...
        // 对于非常小的屏幕，优先考虑垂直方向以获得更大的显示空间
        if (isSmallScreen) {
            // 小屏幕上优先考虑上下位置，因为宽度有限
            finalDirection = ['top', 'bottom'].includes(direction) ? direction : 
                             (markerY > viewportHeight / 2) ? 'top' : 'bottom';
        }
        ...
        if ((finalDirection === 'right' && rightSpace < contentWidth) ||
            (finalDirection === 'left' && leftSpace < contentWidth) ||
            (finalDirection === 'top' && topSpace < contentHeight) ||
            (finalDirection === 'bottom' && bottomSpace < contentHeight)) {
        ...
        // 水平方向边界检查和调整 - 增加边距
        const horizontalMargin = isSmallScreen ? 5 : 10;
        if (viewportPosX < horizontalMargin) {
            viewportPosX = horizontalMargin;
        } else if (viewportPosX + contentWidth > viewportWidth - horizontalMargin) {
            viewportPosX = viewportWidth - contentWidth - horizontalMargin;
        }

        // 垂直方向边界检查和调整 - 增加边距
        const verticalMargin = isSmallScreen ? 5 : 10;
        if (viewportPosY < verticalMargin) {
            viewportPosY = verticalMargin;
        } else if (viewportPosY + contentHeight > viewportHeight - verticalMargin) {
            viewportPosY = viewportHeight - contentHeight - verticalMargin;
        }
...
.annotation-content {
    ...
    z-index: 99990; /* 提高z-index，确保在其他元素之上但低于批注标记 */
    opacity: 0;
    visibility: hidden;
...
/* 小屏幕适配样式 */
.annotation-content.small-screen {
    padding: 10px;
    min-width: 250px;
    font-size: 0.95em;
}
...
@media (max-width: 500px) {
    ...
    .annotation-content .annotation-title {
        font-size: 14px;
    }
